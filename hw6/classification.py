...
def plot_score(train_sc, val_sc=None):
    plt.plot(train_sc, label='training accuracy')
    if val_sc is not None:
        plt.plot(val_sc, label='validation accuracy')
    plt.legend()
    plt.savefig('2.png')
...
def train(mymodel, num_epochs, train_dataloader, validation_dataloader, device, lr):
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            predictions = output.logits
            model_loss = loss(F.softmax(predictions, dim=1), batch['labels'].to(device))
        ...
        cur_train_acc = train_accuracy.compute()
        print(f" - Average training metrics: accuracy={cur_train_acc}")
        train_sc.append(cur_train_acc['accuracy'])

        # normally, validation would be more useful when training for many epochs
        val_accuracy = evaluate_model(mymodel, validation_dataloader, device)
        print(f" - Average validation metrics: accuracy={val_accuracy}")
        val_sc.append(val_accuracy['accuracy'])

        if epoch in [5-1, 7-1, 9-1]:
            test_accuracy = evaluate_model(mymodel, test_dataloader, device)
            print(f" - Average testing metrics: accuracy={test_accuracy}")

    plot_score(train_sc, val_sc)
